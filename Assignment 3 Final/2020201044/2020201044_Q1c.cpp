...
void printTable(suffixString table[], int n, string word)
{
    ...
    for (int i = 0; i < n; i++)
    {
        cout <<i<<" "<< table[i].offset << " " << word.substr(table[i].offset) << " " << table[i].prefixLength << " " << table[i].lengthOfString << endl;
    }
}
...
string longestSubstringPalindrome(suffixString table[], int totLength, string word, int WORD_LENGTH)
{
    ...
    string sub = "";
    int maxIndex = 2;
    for (int i = 2; i < totLength-1; i++)
    {
        // string 1 will have offset from 0 to a-1
        // string 2 will have offset from a+1 to a+b-1
        int gap = 1;
        int flag = 0;
        //while(table[gap+i].prefixLength>0){
        while (((table[i].offset < WORD_LENGTH && table[i + gap].offset > WORD_LENGTH) || (table[i + gap].offset < WORD_LENGTH && table[i].offset > WORD_LENGTH)))
        {
            flag =1;
            if (table[i].offset - 0 == (totLength - (table[i + gap].offset + table[i+gap].prefixLength + 1)))
            {
                if (table[i+gap].prefixLength > maxPrefix)
                {
                    ...
                    maxPrefix = table[i+gap].prefixLength;
                    maxIndex = i+gap;
                }
            }
